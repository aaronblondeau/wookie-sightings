...
// // For base64 file uploader example
...
export const fileUploadProcedures = {
  urlForKey: async (input: any): Promise<string> => {
    return process.env.S3_CDN_URL + "/" + input.key;
  },
  getFileUploadUrl: async (input: any): Promise<{ key: string; url: string }> => {
    if (!input.sightingId) {
  ...
  // // Not used due to payload size restrictions
  // uploadFile: async (input: any): Promise<FileUpload> => {
  //   if (!input.sightingId) {
  //     throw new Error("sighting id must be provided");
  //   }
  //   if (!process.env.S3_BUCKET) {
